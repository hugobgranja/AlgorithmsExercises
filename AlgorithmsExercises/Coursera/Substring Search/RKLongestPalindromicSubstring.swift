...
class RKLongestPalindromicSubstring {
    ...
    private func leps(text: String) -> Substring {
        var longestLength = 1
        var longestIndex = 0
        ...
        return text.subsequence(from: longestIndex, length: longestLength)
    }
    
    // Odd length palindromes
    private func lops(text: String) -> Substring {
        var longestLength = 1
        var longestIndex = 0
    ...
    private func search(_ text: String, length: Int) -> Int? {
        ...
        for i in length..<text.count {
            let leadingValue = UInt64(text.asciiValue(at: i - length)!)
